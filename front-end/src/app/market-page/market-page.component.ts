    ...
    this.marketService.GetCompanyByCompetitionId(window.sessionStorage.getItem('competitionId')).subscribe(data => {
      ...
      data.forEach(element => {
        ...
        if (element.Company != null) {
          ...
          this.market.push({id: element.Company.id, position: this.ite,
          name: element.Company.companyName, price: element.Company.companyCurrentStockPrice,
          gain: (element.Company.companyCurrentStockPrice - element.Company.StockValues[0].stockValue) /
          element.Company.StockValues[0].stockValue * 100});
        }
      });
